// From: https://github.com/YionoFr/GoogleScriptSQL/blob/master/code.js
// See documentation here: https://docs.yiono.fr/
...
function checkIfIDIsExisting(db, table) {
        var value = SpreadsheetApp.getActiveSpreadsheet().getSheetByName(table).getRange(2, 1).getValue()
        if (value === "") {
            return -1
        } else {
            return SpreadsheetApp.getActiveSpreadsheet().getSheetByName(table).getRange(SpreadsheetApp.getActiveSpreadsheet().getSheetByName(table).getLastRow(), 1).getValue()
        }
}
